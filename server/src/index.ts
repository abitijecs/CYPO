...
import { TOPICS, DEBATE_PHASES } from "./topics.js";
...
// ─── System prompt ────────────────────────────────────────────────────────────
const SYSTEM_PROMPT = `Tu es un philosophe-débatteur érudit, impartial et rigoureux.
...
app.post("/api/debate", async (req, res) => {
  const { topicId, phaseId, messages, userMessage }: DebateRequest = req.body;

  if (!topicId || !phaseId || !userMessage) {
    return res.status(400).json({ error: "topicId, phaseId and userMessage are required" });
  }

  const topic = TOPICS.find((t) => t.id === topicId);
  if (!topic) return res.status(404).json({ error: "Topic not found" });

  const phase = DEBATE_PHASES.find((p) => p.id === phaseId);
  if (!phase) return res.status(404).json({ error: "Phase not found" });

  // Build context message for current phase
...
app.listen(PORT, () => {
  console.log(`🏛️  CYPO Debate Server running on http://localhost:${PORT}`);
});
